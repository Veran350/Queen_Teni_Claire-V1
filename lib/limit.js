...
exports.isLimit = function(sender, isPremium, isOwner, limitCount, _db){
    ...
    if (found === false) {
        ...
        _db.push(obj)
        fs.writeFileSync('./user/limit.json', JSON.stringify(_db, null, 2))
        return false
    }
}
...
exports.getLimit = function(sender, limitCount, _db){
    if (_db && typeof _db === 'object' && Object.keys(_db).length > 0) {
        let found = false;
        Object.keys(_db).forEach((i) => {
            if (_db[i].id === sender) {
                found = i;
...
exports.addBalance = function(sender, duit, _db){
    ...
    if (position !== false) {
        ...
        fs.writeFileSync('./user/balance.json', JSON.stringify(_db, null, 2))
    } else {
        const bulin = ({
            id: sender,
            balance: duit
...
exports.isGame = function(sender, isOwner, gcount, _db){
    ...
    if (found === false){
        ...
        _db.push(obj);
        fs.writeFileSync('./user/glimit.json',JSON.stringify(_db, null, 2));
        return false;
    }
}
...
exports.createHit = function(sender, _db){
    ...
    _db.push(anohoh);
    fs.writeFileSync('./user/userhit.json',JSON.stringify(_db, null, 2));''
